...
REPAIR_PLAYBOOKS: Tuple[RepairPlaybook, ...] = (
    ...
    RepairPlaybook(
        playbook_id="recover_sqlite_lock",
        title="Recover temporary SQLite lock safely",
        allowed_actions=("inspect_sqlite_state", "refresh_world_state", "re-run_smoke"),
        required_prechecks=("sqlite_lock_confirmed",),
        verification_steps=(
            VerificationStep("smoke_checks", "Прогнать smoke-check", verifier_kind="smoke_check"),
            VerificationStep("behavioral_checks", "Прогнать behavioral-check", verifier_kind="behavioral_check"),
        ),
        rollback_steps=("Rollback не нужен: сценарий не меняет схему и не трогает данные напрямую.",),
        claim_policy="Можно говорить только о снятии временного lock-состояния после проверок.",
        handles_signals=("sqlite_lock",),
        target_problem_type="sqlite_error",
        preconditions=("sqlite_lock_confirmed",),
        actions=(
            RepairAction("inspect_sqlite_state", "Проверить локальное состояние SQLite", action_kind="internal"),
            RepairAction("refresh_runtime_state", "Обновить runtime/world-state после lock-инцидента", action_kind="internal"),
            RepairAction("run_smoke_check", "Прогнать smoke-check", action_kind="command", command=("python3", "tools/smoke_check.py"), timeout_seconds=120),
            RepairAction("run_behavioral_check", "Прогнать behavioral-check", action_kind="command", command=("python3", "tools/behavioral_check.py"), timeout_seconds=120),
        ),
        expected_effect="Временная lock-ошибка должна исчезнуть без ручного вмешательства в БД.",
        timeout_seconds=180,
    ...
    RepairPlaybook(
        playbook_id="reinitialize_missing_runtime_artifact",
        title="Recreate missing runtime heartbeat artifact",
        allowed_actions=("refresh_heartbeat", "refresh_world_state"),
        required_prechecks=("runtime_artifact_missing_confirmed",),
        verification_steps=(
            VerificationStep("heartbeat_fresh", "Проверить, что heartbeat снова существует и свежий", verifier_kind="heartbeat_fresh"),
            VerificationStep("world_state_fresh", "Проверить, что runtime state больше не stale", verifier_kind="world_state_fresh"),
        ),
        rollback_steps=("Rollback не нужен: heartbeat artifact можно безопасно пересоздать.",),
        claim_policy="Можно говорить только о восстановлении runtime artifact после filesystem verification.",
        handles_signals=("missing_runtime_artifact",),
        target_problem_type="runtime_degraded",
        preconditions=("runtime_artifact_missing_confirmed",),
        actions=(
            RepairAction("reinitialize_heartbeat", "Пересоздать heartbeat artifact", action_kind="internal"),
            RepairAction("refresh_runtime_state", "Обновить runtime/world-state после восстановления artifact", action_kind="internal"),
        ),
        expected_effect="Heartbeat artifact должен снова существовать и считаться свежим.",
        timeout_seconds=30,
        retry_policy="single_retry",
        risk_level="low",
        autonomy_level=AUTONOMY_SAFE_AUTO,
