    ...
    try:
        ...
        if not rows:
            ...
            for created_at, row_user_id, username, first_name, last_name, role, message_type, content in rows:
                ...
                actor = bridge.build_actor_name(row_user_id, username or "", first_name or "", last_name or "", role)
                if role == "user":
                    participant_counts[actor] = participant_counts.get(actor, 0) + 1
                transcript_lines.append(
                    f"[{stamp}] {actor} [{message_type}]: {bridge.truncate_text(bridge.normalize_whitespace(content), 220)}"
                )
            ...
            troublemaker_summary = bridge.render_chat_troublemaker_summary(rows)
            prompt = (
        ...
        bridge.log_exception(f"recent chat report failed chat={chat_id}", error, limit=10)
