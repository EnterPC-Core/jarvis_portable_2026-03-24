...
def _build_chat_watch_truthfulness_footer(*, rows: List[tuple], from_stamp: str, to_stamp: str) -> str:
    ...
    del rows, from_stamp, to_stamp
    timestamp = datetime.now(ZoneInfo("Europe/Moscow")).strftime("%Y-%m-%d %H:%M:%S MSK")
    return "\n".join(
        [
            "",
            "Основано только на текущем окне сообщений, не на всей истории чата.",
            "",
            f"🕒 `{timestamp}`",
        ]
    )
...
def _normalize_chat_watch_report_answer(answer: str) -> str:
    ...
    if not cleaned:
        return ""
    replacements = (
        ("Статус: по последним 100 сообщениям за период", "Статус:"),
        ("Статус: разобрана выборка из 100 сообщений чата", "Статус:"),
        ...
        ("5. Что важно прямо сейчас", "5. Что по сути сейчас обсуждают"),
        ("5. Что сейчас обсуждают practically", "5. Что по сути сейчас обсуждают"),
    )
    for source, target in replacements:
        cleaned = cleaned.replace(source, target)
    cleaned = cleaned.replace("Кто гонит беса", "Что подтверждено / что пока только предположение")
    cleaned = re.sub(r"(?ms)\n?6\.\s*Границы и уверенность.*$", "", cleaned).strip()
    cleaned = re.sub(r"(?m)^\s*(Напрямую видно:|Вывод:|Неопределённость:)\s*", "", cleaned)
    ...
    try:
        ...
        if not rows:
            ...
            prompt = (
                ...
                "Не добавляй отдельный блок про границы анализа, уверенность, размер выборки или число сообщений.\n"
                "Не повторяй в теле ответа формулировки вроде '100 сообщений' или 'выборка из 100 сообщений'.\n"
