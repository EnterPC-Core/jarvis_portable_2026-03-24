...
## ВАЖНО: В ПРОЕКТЕ ДВЕ РАЗДЕЛЬНЫЕ ПЕРСОНЫ

- `Jarvis` и `Enterprise` это не одно и то же.
- `Jarvis` это отдельная persona для обычного диалога.
- `Enterprise` это отдельная persona для server/runtime-задач.
- Логика, контекст, prompt-слой и маршрут у них могут отличаться.
- Изменения для `Enterprise` не должны ломать `Jarvis`.
- Изменения для `Jarvis` не должны подмешиваться в `Enterprise`.
...
- chat-analysis ответы теперь обязаны отдельно помечать прямые наблюдения, выводы и неопределённость, без унижающих ярлыков в обычном аналитическом режиме
