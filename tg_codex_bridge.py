...
UPGRADE_REQUEST_TEMPLATE = """Ты работаешь внутри проекта Telegram ↔ Codex bridge (Python, Termux).
...
- работать только через текущую логику codex exec
...
JARVIS_OFFLINE_TEXT = "Jarvis выключен."
    ...
    def run_codex(self, prompt: str, image_path: Optional[Path] = None, sandbox_mode: Optional[str] = None, approval_policy: Optional[str] = None, json_output: bool = False, postprocess: bool = True) -> str:
        ...
        if result.returncode != 0 and "No prompt provided" in stderr:
            ...
            try:
                ...
                return "Не удалось запустить локальный Codex."
...
def build_help_panel_text(section: str) -> str:
    ...
    panels = {
        ...
        "creator": (
            ...
            "• если Codex недоступен, бот пишет: Jarvis выключен"
        ),
    }
