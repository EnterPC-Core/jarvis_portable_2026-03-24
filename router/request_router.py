...
def detect_local_chat_query(user_text: str, *, normalize_whitespace_func: Callable[[str], str]) -> bool:
    ...
    chat_markers = (
        ...
        "изучи этот чат", "разбери чат", "расскажи про чат", "расскажи про этот чат",
        "про чат", "про этот чат", "весь контекст чата", "весь контекст группы",
        "суть чата", "суть переписки", "разложи по полочкам что происходит",
        "роль в чате", "динамика", "участник", "участники",
    )
...
def response_shape_hint(intent: str) -> str:
    ...
    if intent == "chat_dynamics":
        return (
            "Сначала коротко скажи, что происходит в этом чате сейчас. "
            "Потом разложи ответ по блокам: 1) главная суть, 2) кто задаёт тон и кто самый активный, "
            "3) основные темы/повторы, 4) тон и динамика, 5) что важно понимать про этот чат. "
            "Опирайся на локальный контекст чата, не уходи в новости и не пересказывай лишнее."
        )
