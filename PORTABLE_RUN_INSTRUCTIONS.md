...
- [`services/bridge_context_state.py`](./services/bridge_context_state.py) — event/database retrieval helpers outside monolith state class
...
- [`handlers/control_panel_aux.py`](./handlers/control_panel_aux.py) — extracted panel helper builders
...
Дополнительно retrieval для событий и DB context больше не держится целиком внутри `BridgeState`, что уменьшает coupling portable runtime к одному монолитному entrypoint.
